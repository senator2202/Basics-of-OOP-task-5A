...
import FlowerCompositionPackage.FlowersPackage.Rose;
import FlowerCompositionPackage.Package;

import java.util.Random;

/**
 * Класс для создания композиции из роз, который переопределяет метод суперкласса
 * makeFlowerComposition(), который инициализирует и возвращает
 * объект - цветочную композцию из роз
 */
public class RoseCompositionMaker extends CompositionMaker {
    @Override
    public FlowerComposition makeFlowerComposition() {
        ...
        Flower flower = new Rose(color, r.nextInt(30) + 30);
